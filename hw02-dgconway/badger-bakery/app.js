...
function calculateSubtotal() {
	...
	const total = bagelQuantity * 1.5 + donutQuantity + muffinQuantity * 2 + pieQuantity * 4.5 + cakeQuantity * 7;
	return total;
}

function calculateSalesTax() {
	const stateIn = document.getElementById("stateTax").value;
	const salesTax = SALES_TAX[stateIn];
	const unroundedTax = salesTax * calculateSubtotal();
	return roundMoney(unroundedTax);
}

function calculateTotal() {
	const baseCost = parseFloat(calculateSubtotal());
	const salesTax = parseFloat(calculateSalesTax());
	return (baseCost + salesTax);
}
